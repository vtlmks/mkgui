...
MKGUI_API uint32_t mkgui_message_box(struct mkgui_ctx *ctx, const char *title, const char *message, uint32_t icon_type, uint32_t buttons) {
	...
	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_WINDOW, MKGUI_DLG_WINDOW, "", "", 0, dw, dh, 0, 0, 0);
	strncpy(widgets[wi].label, title, MKGUI_MAX_TEXT - 1);
	++wi;

	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_VBOX, MKGUI_DLG_VBOX, "", "", MKGUI_DLG_WINDOW, 0, 0, 0, 0, 0);
	++wi;

	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_HBOX, MKGUI_DLG_CONTENT_HBOX, "", "", MKGUI_DLG_VBOX, 0, 0, 0, 0, 1);
	++wi;

	if(icon_idx >= 0) {
		widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_PANEL, MKGUI_DLG_ICON_PANEL, "", "", MKGUI_DLG_CONTENT_HBOX, icon_w + DLG_ICON_GAP, 0, MKGUI_FIXED, 0, 0);
		++wi;
	}

	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_VBOX, MKGUI_DLG_TEXT_VBOX, "", "", MKGUI_DLG_CONTENT_HBOX, 0, 0, MKGUI_NO_PAD, 0, 1);
	++wi;
	...
	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_DIVIDER, MKGUI_DLG_DIVIDER, "", "", MKGUI_DLG_VBOX, 0, 0, MKGUI_FIXED, 0, 0);
	++wi;

	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_HBOX, MKGUI_DLG_BTN_HBOX, "", "", MKGUI_DLG_VBOX, 0, DLG_BTN_H, MKGUI_FIXED, 0, 0);
	++wi;

	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_LABEL, MKGUI_DLG_SPACER, "", "", MKGUI_DLG_BTN_HBOX, 0, 0, 0, 0, 1);
	++wi;

	for(uint32_t i = 0; i < btn_count; ++i) {
		widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_BUTTON, MKGUI_DLG_BTN_1 + i, "", "", MKGUI_DLG_BTN_HBOX, DLG_BTN_W, 0, MKGUI_FIXED, 0, 0);
...
MKGUI_API uint32_t mkgui_input_dialog(struct mkgui_ctx *ctx, const char *title, const char *prompt, const char *default_text, char *out, uint32_t out_size) {
	...
	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_INPUT, MKGUI_DLG_INPUT, "", "", MKGUI_DLG_TEXT_VBOX, 0, DLG_INPUT_H, MKGUI_FIXED, 0, 0);
	++wi;
	...
	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_BUTTON, MKGUI_DLG_BTN_1, "OK", "", MKGUI_DLG_BTN_HBOX, DLG_BTN_W, 0, MKGUI_FIXED, 0, 0);
	++wi;

	widgets[wi] = (struct mkgui_widget)MKGUI_W(MKGUI_BUTTON, MKGUI_DLG_BTN_2, "Cancel", "", MKGUI_DLG_BTN_HBOX, DLG_BTN_W, 0, MKGUI_FIXED, 0, 0);
	++wi;
