...
static struct mkgui_window *create_and_layout(struct mkgui_widget *widgets, uint32_t count, struct mkgui_ctx **out_ctx) {
	*out_ctx = mkgui_ctx_create();
	struct mkgui_window *win = mkgui_window_create(*out_ctx, NULL, widgets, count, NULL, 0, 0);
	if(!win) {
		fprintf(stderr, "FATAL: mkgui_window_create returned NULL\n");
		mkgui_ctx_destroy(*out_ctx);
		*out_ctx = NULL;
		return NULL;
	}
	layout_widgets(win);
...
static struct mkgui_rect get_rect(struct mkgui_window *win, uint32_t id) {
	int32_t idx = find_widget_idx(win, id);
	if(idx < 0) {
		struct mkgui_rect zero = {0, 0, 0, 0};
		return zero;
	}
	return win->rects[idx];
}

static void check_no_overlap_siblings(struct mkgui_window *win, uint32_t parent_id, char *test_name, uint32_t *fail) {
	uint32_t children[256];
	uint32_t child_count = 0;
	for(uint32_t i = 0; i < win->widget_count; ++i) {
		if(win->widgets[i].parent_id == parent_id && !(win->widgets[i].flags & MKGUI_HIDDEN)) {
			if(child_count < 256) {
	...
	for(uint32_t a = 0; a < child_count; ++a) {
		struct mkgui_rect ra = win->rects[children[a]];
		for(uint32_t b = a + 1; b < child_count; ++b) {
			struct mkgui_rect rb = win->rects[children[b]];
			...
			if(overlap_x && overlap_y && ra.w > 0 && ra.h > 0 && rb.w > 0 && rb.h > 0) {
				fprintf(stderr, "  overlap: widget[%u] id=%u (%d,%d %dx%d) vs widget[%u] id=%u (%d,%d %dx%d)\n", children[a], win->widgets[children[a]].id, ra.x, ra.y, ra.w, ra.h, children[b], win->widgets[children[b]].id, rb.x, rb.y, rb.w, rb.h);
				*fail = 1;
			}
		}
	}
...
static void check_children_inside_parent(struct mkgui_window *win, uint32_t parent_id, char *test_name, uint32_t *fail) {
	int32_t pi = find_widget_idx(win, parent_id);
	if(pi < 0) {
		return;
	}
	struct mkgui_rect pr = win->rects[pi];
	for(uint32_t i = 0; i < win->widget_count; ++i) {
		if(win->widgets[i].parent_id == parent_id && !(win->widgets[i].flags & MKGUI_HIDDEN)) {
			struct mkgui_rect cr = win->rects[i];
			if(cr.w <= 0 || cr.h <= 0) {
				continue;
			}

			if(cr.x < pr.x || cr.y < pr.y || cr.x + cr.w > pr.x + pr.w || cr.y + cr.h > pr.y + pr.h) {
				fprintf(stderr, "  outside parent: widget[%u] id=%u (%d,%d %dx%d) parent id=%u (%d,%d %dx%d)\n", i, win->widgets[i].id, cr.x, cr.y, cr.w, cr.h, parent_id, pr.x, pr.y, pr.w, pr.h);
				*fail = 1;
			}
		}
	}
...
static void test_vbox_basic(void) {
	...
	struct mkgui_window *win = create_and_layout(widgets, 5, &ctx);
	CHECK(win, "create failed");
	if(win) {
		for(uint32_t id = BTN1; id <= BTN3; ++id) {
			struct mkgui_rect r = get_rect(win, id);
			CHECK(r.w > 0, "id=%u w=%d", id, r.w);
			CHECK(r.h > 0, "id=%u h=%d", id, r.h);
		}
		check_no_overlap_siblings(win, VBOX1, test_name__, &fail__);
		check_children_inside_parent(win, VBOX1, test_name__, &fail__);
...
static void test_hbox_basic(void) {
	...
	if(win) {
		...
		check_no_overlap_siblings(win, HBOX1, test_name__, &fail__);
		check_children_inside_parent(win, HBOX1, test_name__, &fail__);
		mkgui_window_destroy(win);
		mkgui_ctx_destroy(ctx);
	}
	TEST_END();
...
static void test_weights(void) {
	...
	struct mkgui_ctx *ctx;
	struct mkgui_window *win = create_and_layout(widgets, 4, &ctx);
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect r1 = get_rect(win, LBL1);
		struct mkgui_rect r2 = get_rect(win, LBL2);
		...
		int32_t nh = natural_height(win, MKGUI_LABEL);
...
static void test_fixed_width(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect r1 = get_rect(win, BTN1);
		struct mkgui_rect r2 = get_rect(win, BTN2);
...
static void test_hidden_no_space(void) {
	...
	if(win) {
		struct mkgui_rect r1 = get_rect(win, BTN1);
		struct mkgui_rect r3 = get_rect(win, BTN3);
...
static void test_chrome_no_overlap(void) {
	...
	struct mkgui_ctx *ctx;
	struct mkgui_window *win = create_and_layout(widgets, 8, &ctx);
	CHECK(win, "create failed");
	if(win) {
		mkgui_statusbar_setup(win, SB, 1, (int32_t[]){-1});
		layout_widgets(win);
		struct mkgui_rect menu_r = get_rect(win, MENU1);
		struct mkgui_rect tb_r   = get_rect(win, TB);
		struct mkgui_rect sb_r   = get_rect(win, SB);
		struct mkgui_rect vbox_r = get_rect(win, VBOX1);
...
static void test_split_children_positive(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect lr = get_rect(win, LEFT);
		struct mkgui_rect rr = get_rect(win, RIGHT);
...
static void test_hsplit_children_positive(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect tr = get_rect(win, TOP);
		struct mkgui_rect br = get_rect(win, BOT);
...
static void test_many_children_no_zero(void) {
	...
	struct mkgui_ctx *ctx;
	struct mkgui_window *win = create_and_layout(widgets, 22, &ctx);
	CHECK(win, "create failed");
	if(win) {
		for(uint32_t i = 0; i < 20; ++i) {
			struct mkgui_rect r = get_rect(win, 10 + i);
...
static void test_small_window_no_zero(void) {
	...
	struct mkgui_ctx *ctx;
	struct mkgui_window *win = create_and_layout(widgets, 6, &ctx);
	CHECK(win, "create failed");
	if(win) {
		for(uint32_t id = BTN1; id <= BTN3; ++id) {
			struct mkgui_rect r = get_rect(win, id);
		...
		struct mkgui_rect lv_r = get_rect(win, LV);
...
static void test_form_layout(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect l1 = get_rect(win, LBL1);
		struct mkgui_rect i1 = get_rect(win, INP1);
		struct mkgui_rect l2 = get_rect(win, LBL2);
		...
		check_children_inside_parent(win, FORM1, test_name__, &fail__);
		mkgui_window_destroy(win);
		mkgui_ctx_destroy(ctx);
	}
	TEST_END();
...
static void test_hbox_many_children_no_zero(void) {
	...
	struct mkgui_ctx *ctx;
	struct mkgui_window *win = create_and_layout(widgets, 17, &ctx);
	CHECK(win, "create failed");
	if(win) {
		for(uint32_t i = 0; i < 15; ++i) {
			struct mkgui_rect r = get_rect(win, 10 + i);
...
static void test_mixed_fixed_flex(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect b1 = get_rect(win, BTN1);
		struct mkgui_rect lv = get_rect(win, LV);
		struct mkgui_rect b2 = get_rect(win, BTN2);
...
static void test_tabs_content_inside(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect tabs_r = get_rect(win, TABS1);
		struct mkgui_rect btn1_r = get_rect(win, BTN1);
...
static void test_full_chrome_stack(void) {
	...
	struct mkgui_ctx *ctx;
	struct mkgui_window *win = create_and_layout(widgets, 9, &ctx);
	CHECK(win, "create failed");
	if(win) {
		mkgui_statusbar_setup(win, SB, 1, (int32_t[]){-1});
		layout_widgets(win);
		...
		struct mkgui_rect btn_r = get_rect(win, BTN1);
...
static void test_panel_border(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		check_children_inside_parent(win, PANEL1, test_name__, &fail__);
		check_no_overlap_siblings(win, PANEL1, test_name__, &fail__);
		mkgui_window_destroy(win);
		mkgui_ctx_destroy(ctx);
	}
	TEST_END();
...
static void test_no_widget_zero_size(void) {
	...
	if(win) {
		for(uint32_t i = 0; i < win->widget_count; ++i) {
			if(win->widgets[i].flags & MKGUI_HIDDEN) {
				continue;
			}
			...
			struct mkgui_rect r = win->rects[i];
			CHECK(r.w > 0, "widget[%u] id=%u type=%u w=%d", i, win->widgets[i].id, win->widgets[i].type, r.w);
			CHECK(r.h > 0, "widget[%u] id=%u type=%u h=%d", i, win->widgets[i].id, win->widgets[i].type, r.h);
		}
		mkgui_window_destroy(win);
		mkgui_ctx_destroy(ctx);
	}
...
static void test_listview_min_height(void) {
	...
	if(win) {
		struct mkgui_rect lv_r = get_rect(win, LV);
		int32_t min_h = win->row_height * 3;
...
static void test_textarea_min_height(void) {
	...
	struct mkgui_ctx *ctx;
	struct mkgui_window *win = create_and_layout(widgets, 7, &ctx);
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect ta_r = get_rect(win, TA);
		int32_t min_h = win->row_height * 3;
...
static void test_hbox_min_width(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		int32_t min_w = natural_width(win, find_widget(win, 10));
		for(uint32_t i = 0; i < 20; ++i) {
			struct mkgui_rect r = get_rect(win, 10 + i);
...
static void test_tabs_min_height(void) {
	...
	if(win) {
		struct mkgui_rect tabs_r = get_rect(win, TABS1);
		int32_t min_h = win->tab_height + win->row_height * 2;
...
static void test_split_pane_min_height(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect split_r = get_rect(win, SPLIT);
		int32_t min_h = win->split_min_px * 2 + win->split_thick;
...
static void test_hbox_label_progress_spinner(void) {
	...
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect hbox_r = get_rect(win, HBOX1);
		struct mkgui_rect lbl_r = get_rect(win, LBL);
		struct mkgui_rect pg_r = get_rect(win, PG);
		struct mkgui_rect sp_r = get_rect(win, SP);
...
static void test_hbox_shrink_stays_inside(void) {
	...
	if(win) {
		...
		check_no_overlap_siblings(win, HBOX1, test_name__, &fail__);
		struct mkgui_rect lbl_r = get_rect(win, LBL);
		struct mkgui_rect inp_r = get_rect(win, INP);
		struct mkgui_rect cb_r = get_rect(win, CB);
...
static void test_form_shrink_no_overlap(void) {
	...
	if(win) {
		...
		struct mkgui_rect l3 = get_rect(win, LBL3);
		struct mkgui_rect cb_r = get_rect(win, CB);
...
static void test_hbox_fixed_zero_width(void) {
	...
	if(win) {
		struct mkgui_rect r1 = get_rect(win, BTN1);
		int32_t nw = natural_width(win, find_widget(win, BTN1));
...
static void test_generic_fixed_zero_size(void) {
	...
	struct mkgui_ctx *ctx;
	struct mkgui_window *win = create_and_layout(widgets, 2, &ctx);
	CHECK(win, "create failed");
	if(win) {
		struct mkgui_rect win_r = get_rect(win, WIN);
		struct mkgui_rect btn_r = get_rect(win, BTN1);
		struct mkgui_widget *bw = find_widget(win, BTN1);
		int32_t nw = natural_width(win, bw);
		int32_t nh = natural_height(win, MKGUI_BUTTON);
