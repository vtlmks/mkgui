...
// [=]===^=[ render_datepicker ]===================================[=]
static void render_datepicker(struct mkgui_window *win, uint32_t idx) {
	...
	uint32_t border = disabled_blend(focused ? win->theme.highlight : win->theme.widget_border, win->theme.bg, disabled);
	uint32_t input_bg = disabled_blend(win->theme.input_bg, win->theme.bg, disabled);

	int32_t btn_w = sc(win, DATEPICKER_BTN_W_PX);
	...
	struct mkgui_datepicker_data *dp = find_datepicker_data(win, w->id);
	if(!dp) {
		return;
	}
	...
	if(focused && dp->editing) {
		...
		tmp[cpos] = '\0';
		int32_t cx = rx + text_pad + text_width(win, tmp);
		draw_vline(win->pixels, win->win_w, win->win_h, cx, ry + cursor_inset, rh - cursor_inset * 2, win->theme.text);
	} else {
		char buf[16];
	...
	int32_t ic_w = sc(win, 12);
	int32_t ic_h = sc(win, 10);
	int32_t icon_x = rx + text_w + (btn_w - ic_w) / 2;
	int32_t icon_y = ry + (rh - ic_h) / 2;
	int32_t ic_dot = sc(win, 2);
	int32_t ic_hdr = sc(win, 3);
	draw_rect_fill(win->pixels, win->win_w, win->win_h, icon_x, icon_y, ic_w, ic_h, tc);
	draw_rect_fill(win->pixels, win->win_w, win->win_h, icon_x + 1, icon_y + ic_hdr, ic_w - 2, ic_h - ic_hdr - 1, win->theme.input_bg);
	int32_t ic_dx = sc(win, 3);
	int32_t ic_dy = sc(win, 1);
	draw_rect_fill(win->pixels, win->win_w, win->win_h, icon_x + ic_dot, icon_y + ic_hdr + ic_dy, ic_dot, ic_dot, tc);
	draw_rect_fill(win->pixels, win->win_w, win->win_h, icon_x + ic_dot + ic_dx, icon_y + ic_hdr + ic_dy, ic_dot, ic_dot, tc);
	draw_rect_fill(win->pixels, win->win_w, win->win_h, icon_x + ic_dot + ic_dx * 2, icon_y + ic_hdr + ic_dy, ic_dot, ic_dot, tc);
	draw_rect_fill(win->pixels, win->win_w, win->win_h, icon_x + ic_dot, icon_y + ic_hdr + ic_dy + ic_dx, ic_dot, ic_dot, tc);
	draw_rect_fill(win->pixels, win->win_w, win->win_h, icon_x + ic_dot + ic_dx, icon_y + ic_hdr + ic_dy + ic_dx, ic_dot, ic_dot, tc);
}

// [=]===^=[ render_datepicker_popup ]=============================[=]
static void render_datepicker_popup(struct mkgui_window *win, struct mkgui_popup *p, struct mkgui_datepicker_data *dp, int32_t hover_day) {
	...
	int32_t hdr_h = sc(win, DATEPICKER_HDR_H_PX);
	int32_t dow_h = sc(win, DATEPICKER_DOW_H_PX);
	int32_t cell_size = sc(win, DATEPICKER_CELL_SIZE_PX);
	int32_t popup_pad = sc(win, 4);
	int32_t gap2 = sc(win, 2);
	int32_t text_pad = sc(win, 4);

	int32_t cy = popup_pad;
	int32_t arrow_w = sc(win, 20);
	int32_t hdr_y = cy + (hdr_h - win->font_height) / 2;
	...
	int32_t year_spin_w = sc(win, 56);
	int32_t year_btn_w = sc(win, 14);
	...
	uint32_t lbg = left_hover ? win->theme.widget_hover : win->theme.menu_bg;
	draw_rounded_rect_fill(p->pixels, p->w, p->h, left_x, btn_y, arrow_w, btn_h, lbg, win->theme.corner_radius);
	int32_t acx_l = left_x + arrow_w / 2;
	int32_t arr_half = sc(win, 3);
	int32_t arr_off = sc(win, 2);
	for(uint32_t j = 0; j < (uint32_t)(arr_half + 1); ++j) {
		draw_vline(p->pixels, p->w, p->h, acx_l + arr_off - (int32_t)j, acy - arr_half + (int32_t)j, 1, win->theme.text);
		draw_vline(p->pixels, p->w, p->h, acx_l + arr_off - (int32_t)j, acy + arr_half - (int32_t)j, 1, win->theme.text);
	}

	uint32_t rbg = right_hover ? win->theme.widget_hover : win->theme.menu_bg;
	draw_rounded_rect_fill(p->pixels, p->w, p->h, right_x, btn_y, arrow_w, btn_h, rbg, win->theme.corner_radius);
	int32_t acx_r = right_x + arrow_w / 2;
	for(uint32_t j = 0; j < (uint32_t)(arr_half + 1); ++j) {
		draw_vline(p->pixels, p->w, p->h, acx_r - arr_off + (int32_t)j, acy - arr_half + (int32_t)j, 1, win->theme.text);
		draw_vline(p->pixels, p->w, p->h, acx_r - arr_off + (int32_t)j, acy + arr_half - (int32_t)j, 1, win->theme.text);
	}

	int32_t drop_icon_w = sc(win, 12);
	uint32_t mbg = month_hover ? win->theme.widget_hover : win->theme.widget_bg;
	draw_rounded_rect(p->pixels, p->w, p->h, inner_x, btn_y, month_w, btn_h, mbg, win->theme.widget_border, win->theme.corner_radius);
	int32_t mname_tw = text_width(win, mname);
	push_text_clip(p->x + inner_x + text_pad, hdr_y + p->y, mname, win->theme.text, p->x + inner_x, p->y, p->x + inner_x + month_w - drop_icon_w, p->y + p->h);
	int32_t drop_acx = inner_x + month_w - drop_icon_w + arr_half;
	int32_t drop_acy = acy - gap2 + arr_half / 2;
	draw_triangle_aa(p->pixels, p->w, p->h, drop_acx - arr_half, drop_acy - arr_half / 2, drop_acx + arr_half, drop_acy - arr_half / 2, drop_acx, drop_acy + arr_half / 2, win->theme.text);
	(void)mname_tw;

	int32_t yr = win->theme.corner_radius;
	int32_t year_half = btn_h / 2;
	draw_rounded_rect(p->pixels, p->w, p->h, year_x, btn_y, year_spin_w, btn_h, win->theme.input_bg, win->theme.widget_border, yr);
	char year_buf[8];
	snprintf(year_buf, sizeof(year_buf), "%d", dp->view_year);
	int32_t year_tw = text_width(win, year_buf);
	int32_t year_text_w = year_spin_w - year_btn_w;
	push_text_clip(p->x + year_x + (year_text_w - year_tw) / 2, hdr_y + p->y, year_buf, win->theme.text, p->x + year_x + 1, p->y, p->x + year_x + year_text_w, p->y + p->h);

	uint32_t up_bg = year_up_hover ? win->theme.widget_hover : win->theme.widget_bg;
	int32_t sbx = year_x + year_spin_w - year_btn_w;
	draw_rounded_rect(p->pixels, p->w, p->h, sbx, btn_y, year_btn_w, year_half, up_bg, win->theme.widget_border, yr);
	int32_t sacx = sbx + year_btn_w / 2;
	int32_t sacy_up = btn_y + year_half / 2 - gap2;
	draw_triangle_aa(p->pixels, p->w, p->h, sacx, sacy_up, sacx - arr_half, sacy_up + arr_half, sacx + arr_half, sacy_up + arr_half, win->theme.text);

	uint32_t dn_bg = year_dn_hover ? win->theme.widget_hover : win->theme.widget_bg;
	draw_rounded_rect(p->pixels, p->w, p->h, sbx, btn_y + year_half, year_btn_w, btn_h - year_half, dn_bg, win->theme.widget_border, yr);
	int32_t sacy_dn = btn_y + year_half + (btn_h - year_half) / 2 - gap2;
	draw_triangle_aa(p->pixels, p->w, p->h, sacx - arr_half, sacy_dn, sacx + arr_half, sacy_dn, sacx, sacy_dn + arr_half, win->theme.text);

	cy += hdr_h;

	if(dp->month_select) {
		for(uint32_t mi = 0; mi < 12; ++mi) {
			int32_t iy = cy + 1 + (int32_t)mi * win->row_height;
			if(iy + win->row_height <= cy || iy >= p->h) {
				continue;
			}
			...
			uint32_t tc = is_sel ? win->theme.sel_text : win->theme.text;
			int32_t list_pad = sc(win, 5);
		...
		for(uint32_t d = 0; d < 7; ++d) {
			...
			int32_t dw = text_width(win, dow[d]);
			push_text_clip(p->x + dx + (cw - dw) / 2, cy + gap2 + p->y, dow[d], win->theme.text_disabled, p->x, p->y, p->x + p->w, p->y + p->h);
		}
		cy += dow_h;
		...
		for(uint32_t row = 0; row < 6; ++row) {
			for(uint32_t col = 0; col < 7; ++col) {
				...
				if(is_selected) {
					draw_rounded_rect_fill(p->pixels, p->w, p->h, center_x - circ_r, center_y - circ_r, circ_r * 2, circ_r * 2, win->theme.accent, circ_r);
				} else if(is_hover) {
					draw_rounded_rect_fill(p->pixels, p->w, p->h, center_x - circ_r, center_y - circ_r, circ_r * 2, circ_r * 2, win->theme.widget_hover, circ_r);
				} else if(is_today) {
					draw_rounded_rect(p->pixels, p->w, p->h, center_x - circ_r, center_y - circ_r, circ_r * 2, circ_r * 2, win->theme.menu_bg, win->theme.accent, circ_r);
				}

				char dbuf[12];
				snprintf(dbuf, sizeof(dbuf), "%d", day_num);
				int32_t dw = text_width(win, dbuf);
				int32_t tx = cx + (cw - dw) / 2;
				int32_t text_y = cell_y + (cw - win->font_height) / 2;
				uint32_t tc;
				if(is_selected) {
					tc = 0xffffffff;
...
static void datepicker_open_popup(struct mkgui_window *win, uint32_t widget_id) {
	struct mkgui_datepicker_data *dp = find_datepicker_data(win, widget_id);
	if(!dp) {
		return;
	}
	...
	int32_t cal_w = sc(win, DATEPICKER_CAL_W_PX);
	int32_t cal_h = sc(win, DATEPICKER_HDR_H_PX) + 12 * win->row_height + sc(win, 4);
	popup_destroy_all(win);
	struct mkgui_popup *p = popup_create(win, abs_x, abs_y, cal_w, cal_h, widget_id);
	if(p) {
		render_datepicker_popup(win, p, dp, -1);
...
// [=]===^=[ datepicker_cal_hit ]=================================[=]
static int32_t datepicker_cal_hit(struct mkgui_window *win, struct mkgui_datepicker_data *dp, int32_t mx, int32_t my) {
	int32_t hdr_h = sc(win, DATEPICKER_HDR_H_PX);
	int32_t dow_h = sc(win, DATEPICKER_DOW_H_PX);
	int32_t cell_sz = sc(win, DATEPICKER_CELL_SIZE_PX);
	int32_t cal_w = sc(win, DATEPICKER_CAL_W_PX);
	int32_t popup_pad = sc(win, 4);
	int32_t gap2 = sc(win, 2);
	int32_t arrow_w = sc(win, 20);
	...
	if(dp->month_select) {
		...
		int32_t item = (my - list_y) / win->row_height;
		if(item >= 0 && item < 12) {
			return item + 100;
		}
		return -1;
...
static void handle_datepicker_click(struct mkgui_window *win, uint32_t widget_id) {
	struct mkgui_datepicker_data *dp = find_datepicker_data(win, widget_id);
	if(!dp) {
		return;
	}
	...
	int32_t btn_x = rx + rw - sc(win, DATEPICKER_BTN_W_PX);

	if(win->mouse_x >= btn_x) {
		datepicker_open_popup(win, widget_id);
	} else {
		dp->editing = 1;
...
static uint32_t handle_datepicker_key(struct mkgui_window *win, struct mkgui_event *ev, uint32_t ks, uint32_t keymod, char *buf, int32_t len) {
	struct mkgui_datepicker_data *dp = find_datepicker_data(win, win->focus_id);
	if(!dp) {
		return 0;
	}
	(void)keymod;
	...
	case MKGUI_KEY_RETURN: {
		...
		datepicker_open_popup(win, win->focus_id);
		return 1;
	}
...
MKGUI_API void mkgui_datepicker_set(struct mkgui_window *win, uint32_t id, int32_t year, int32_t month, int32_t day) {
	MKGUI_CHECK(win);
	struct mkgui_datepicker_data *dp = find_datepicker_data(win, id);
	if(!dp) {
		return;
	}
	dp->year = year;
