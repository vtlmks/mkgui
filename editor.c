...
static struct ed_prop_desc ed_props[] = {
	...
	{ ED_PK_FLAG,          "NoScroll",        offsetof(struct ed_widget, flags),         MKGUI_NO_SCROLL,    0,      0,    ED_VIS_BOX,       ED_ACT_NONE,         ED_PROP_FL_SCROLL,    0,                   0,                 0 },
...
static char *ed_flags_to_str(uint32_t flags, char *buf, uint32_t buf_size) {
	...
	static struct { uint32_t bit; char *name; } single_bits[] = {
		...
		{ MKGUI_NO_SCROLL,       "MKGUI_NO_SCROLL" },
...
static void ed_generate_code(struct mkgui_ctx *ctx) {
	...
	uint32_t has_toolbar = 0;
	char seen_icons[256][MKGUI_ICON_NAME_LEN];
	uint32_t seen_count = 0;
	char seen_tb_icons[64][MKGUI_ICON_NAME_LEN];
	uint32_t seen_tb_count = 0;
	...
	for(uint32_t i = 0; i < ed.widget_count; ++i) {
		...
		if(w->parent_id != 0) {
			int32_t pidx = ed_find_widget(w->parent_id);
			if(pidx >= 0 && ed.widgets[pidx].type == MKGUI_TOOLBAR) {
				is_tb_child = 1;
			}
		}
		char (*arr)[MKGUI_ICON_NAME_LEN] = is_tb_child ? seen_tb_icons : seen_icons;
		uint32_t *cnt = is_tb_child ? &seen_tb_count : &seen_count;
		uint32_t max = is_tb_child ? 64 : 256;
		uint32_t dup = 0;
		for(uint32_t j = 0; j < *cnt; ++j) {
			if(strcmp(arr[j], w->icon) == 0) {
				dup = 1;
				break;
			}
		}
		if(!dup && *cnt < max) {
			snprintf(arr[*cnt], MKGUI_ICON_NAME_LEN, "%s", w->icon);
			++(*cnt);
		}
		if(!is_tb_child) {
			uint32_t tb_dup = 0;
			for(uint32_t j = 0; j < seen_tb_count; ++j) {
				if(strcmp(seen_tb_icons[j], w->icon) == 0) {
					tb_dup = 1;
					break;
				}
			}
			if(!tb_dup && seen_tb_count < 64) {
				snprintf(seen_tb_icons[seen_tb_count], MKGUI_ICON_NAME_LEN, "%s", w->icon);
				++seen_tb_count;
			}
		}
	}
	...
	if(seen_count > 0) {
		FILE *fi = fopen(icon_path, "w");
		if(fi) {
			fprintf(fi, "# Icon subset generated by mkgui editor\n");
			fprintf(fi, "# Add any icons loaded dynamically in your code\n\n");
	...
	if(has_toolbar && seen_tb_count > 0) {
		FILE *ft = fopen(tb_icon_path, "w");
		if(ft) {
			fprintf(ft, "# Toolbar icon subset generated by mkgui editor\n");
			fprintf(ft, "# Add any toolbar icons loaded dynamically in your code\n\n");
			for(uint32_t i = 0; i < seen_tb_count; ++i) {
				fprintf(ft, "%s\n", seen_tb_icons[i]);
			}
			fclose(ft);
		}
