...
// ---------------------------------------------------------------------------
// Icon font rendering (FreeType)
// ---------------------------------------------------------------------------

static FT_Library icon_ft_lib;
static FT_Face icon_ft_face;
...
static uint32_t platform_icon_font_init(const unsigned char *data, size_t size, uint32_t pixel_size) {
	...
	if(FT_Init_FreeType(&icon_ft_lib)) {
		fprintf(stderr, "mkgui: cannot init freetype for icons\n");
		return 0;
	}
	if(FT_New_Memory_Face(icon_ft_lib, data, (FT_Long)size, 0, &icon_ft_face)) {
		fprintf(stderr, "mkgui: cannot load MDI font (%zu bytes)\n", size);
		icon_ft_face = NULL;
		return 0;
	}
	FT_Set_Pixel_Sizes(icon_ft_face, 0, pixel_size);
	return 1;
}
...
static uint32_t platform_icon_render_glyph(uint32_t codepoint, uint32_t color, uint32_t *dst, uint32_t size) {
	FT_UInt glyph_idx = FT_Get_Char_Index(icon_ft_face, codepoint);
	if(!glyph_idx) {
		return 0;
	}
	if(FT_Load_Glyph(icon_ft_face, glyph_idx, FT_LOAD_RENDER)) {
		return 0;
	}

	FT_GlyphSlot slot = icon_ft_face->glyph;
